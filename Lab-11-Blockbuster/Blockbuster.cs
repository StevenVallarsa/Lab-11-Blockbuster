    ...
    public class Blockbuster
    {
        ...
        public void Blockbuster()
        {
            for (int i = 0; i < 6; i++)
            {
                ...
                List<string> movieScenes = new List<string>() { movieInfo[4], movieInfo[5], movieInfo[6], movieInfo[7], movieInfo[8], movieInfo[9] };
                      
                if (i % 2 == 0)
                {
                    DVD temp = new DVD(movieInfo[0], movieInfo[1], int.Parse(movieInfo[2]), movieScenes)
                }
